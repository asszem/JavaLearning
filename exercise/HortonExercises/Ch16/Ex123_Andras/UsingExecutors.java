...
//      Future<int[]> debits = threadPool.submit(new TransactionSource(TransactionType.DEBIT, transactionCount, accounts, clerks));
      Future<int[][]> randomTransactions = threadPool.submit(new TransactionSource(transactionCount, accounts, clerks));
      ...
      try {
    	  //Get the total credits/debits for each account
    	  for (int i=0;i<totalCredits.length;i++){
			totalCredits[i] = randomTransactions.get()[i][0];
			totalDebits[i] = randomTransactions.get()[i][1];
    	  }
