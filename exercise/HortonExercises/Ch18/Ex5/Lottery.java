/*
 * Exercise 5 - Requirement
 * Change the Lottery applet to handle the MOUSE_ENTERED and MOUSE_EXITED events within the
toolbar buttons you added in the previous exercise and display a hand cursor
 * Exercise 6 
 * 
 *  Add tooltips to the lucky number buttons and the toolbar buttons in the Lottery applet. (You can make the
tooltip the same for each of the lucky number buttons.)
 * */
package HortonExercises.Ch18.Ex5;
...
public class Lottery extends JApplet {
	...
	public void createGUI() {
		...
		// Add tooltips for action objects
		changeColor.putValue(SHORT_DESCRIPTION, "Change color");
		getNewNumbers.putValue(SHORT_DESCRIPTION, "Get new numbers");
		...
		for (int i = 0; i < toolBar.getComponentCount(); i++) {
			if (toolBar.getComponent(i) instanceof JButton) {
				...
				temp.addMouseListener(getNewNumbers);
		...
		changeColorButton.addMouseListener(changeColor);
...
//			luckyNumberButtons[i].putV
	...
	private class NumberButton extends JButton implements MouseListener {
		...
		public NumberButton(int value) {
			...
			setToolTipText("Selection button"+this.buttonValue);
		}
	...
	private class HandleActionObject extends AbstractAction implements MouseListener{
