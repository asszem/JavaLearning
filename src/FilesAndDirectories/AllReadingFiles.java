...
public class AllReadingFiles {
	 ...
	 * @param filename
	 * @param readMethod
	 *            1- reads a single char; 2-reads everyting to a char array; 3- reads all lines
	 ...
	 * @param filename
	 * @param readMethod
	 *            Different methods to read, but should produce the same results. method 1 - reads every character into a separate index to the arraylist method 2 - reads all content to one single
	 *            index to the arraylist
	...
	/**
	 * Read all data from a TEXT file using a Scanner with specific Locale and Charset
	 * */
	...
	public static ArrayList readMixedDataFromByteChannel(Path filename, int readMethod) {
		...
		/*
		 * Method 1 - determine the String length in standalone read operation - resource ineffective Method 2 - Create a bigger ByteBuffer, load with data and use the .compact() method when there is
		 * not enough space available Method 3 - Horton's logic. Check buffer remaining before Double/String/Long and if less then read with compact Horton's book, Chapter 11, reading files, page 431
		 * File structure [double][string][long] double = the length of the following string (stored as Double to make things even more complicated :D) string = the prime values as string, example
		 * prime = 23 long = the prime value as long
		 */
		// </editor-fold>
		try (FileChannel readChannel = (FileChannel) Files.newByteChannel(filename, READ);) {
			...
			case 1: // Using two read operations in each iteration
				...
				while (true) { // loop through the file
					...
					formattedResult.format("Sting length:[%03d] String:[%s] Long:[%d]", currentStringLength,
							currentAsString, currentAsLong);
			...
			case 2: // Using the .compact method
				...
				/*
				 * pseudo code At this point we have the buffer with data. What we ASSUME, the first 8 is a double 1. read the first string length (position will be updated in bytebuffer) 2. calculate
				 * the first required length (stringlenght*2+8) 3. make a while cycle until the bytbuffer has enough remaining to get the requied string and long data get the string and long, build
				 * the next result array and increment buffer position after the while cycle compact the buffer, and then read again (but keep the already read data's position)
				 */
				// </editor-fold>
			...
			case 3: // Using the .compact method based on logic in Horton's book
				...
				ByteBuffer buf = ByteBuffer.allocateDirect(36); // A direct buffer is faster if you are reading a lot of data from a file, as the data is transferred directly from the file to our
																// buffer.
				...
				while (true) {
					...
					resultArray.add(formattedResult.format("Sting length:[%03d] String:[%s] Long:[%d]", strLength,
							ByteBuffer.wrap(strChars).asCharBuffer(), longValue));
