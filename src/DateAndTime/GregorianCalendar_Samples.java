...
public class GregorianCalendar_Samples {
	...
	public static void main(String[] args) {
		...
		displayFormattedCalendar(calendar);
		calendar.roll(DAY_OF_MONTH, false); //Rolls back one day, if EOM reached, starts from the END
		displayFormattedCalendar(calendar);
		calendar.roll(DAY_OF_MONTH, true); 	//Rolls forward one day
		displayFormattedCalendar(calendar);
		calendar.roll(DAY_OF_MONTH, true); 	//Rolls forward one day, if EOM reached, starts from the BEGINNIG
		displayFormattedCalendar(calendar);
		calendar.add(DAY_OF_MONTH, -1);     //Updates every field in Calendar object if changed
		displayFormattedCalendar(calendar);
		calendar.roll(Calendar.DAY_OF_MONTH, +10);	//rolls 10 days forward, DOESNT! change other fields
		displayFormattedCalendar(calendar);
